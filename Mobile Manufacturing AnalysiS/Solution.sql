...
DIM_LOCATION INNER JOIN FACT_TRANSACTIONS  ON FACT_TRANSACTIONS.IDLOCATION=DIM_LOCATION.IDLOCATION 
WHERE YEAR(FACT_TRANSACTIONS.Date)>=2005;
-------------------OR-------------------------------
SELECT  DISTINCT State FROM 
DIM_LOCATION LEFT JOIN FACT_TRANSACTIONS  ON FACT_TRANSACTIONS.IDLOCATION=DIM_LOCATION.IDLOCATION 
WHERE YEAR(FACT_TRANSACTIONS.Date)>=2005;
...
WHERE MANUFACTURER_NAME='Samsung' and country='us' group by state
having sum(quantity)=(select max(qty) from 
(select state,sum(quantity) QTY from DIM_LOCATION INNER JOIN FACT_TRANSACTIONS ON FACT_TRANSACTIONS.IDLOCATION=DIM_LOCATION.IDLOCATION 
INNER JOIN  DIM_MODEL ON FACT_TRANSACTIONS.IDMODEL=DIM_MODEL.IDMODEL 
INNER JOIN DIM_MANUFACTURER ON DIM_MANUFACTURER.IDManufacturer=DIM_MODEL.IDMANUFACTURER
where country='US' AND MANUFACTURER_NAME='SAMSUNG' GROUP BY STATE) AS KDKK)


---3.	Show the number of transactions for each model per zip code per state.
SELECT STATE,ZIPCODE,MODEL_NAME,COUNT(FACT_TRANSACTIONS.IDMODEL) FROM 
FACT_TRANSACTIONS INNER JOIN DIM_MODEL ON FACT_TRANSACTIONS.IDMODEL=DIM_MODEL.IDMODEL
...
SELECT MODEL_NAME AS 'Cheapest Cellphone' 
...
-------------------OR-------------------------------
SELECT MODEL_NAME AS 'Cheapest Cellphone'  FROM DIM_MODEL 
...
select manufacturer_name,model_name,avg(totalprice)from 
fact_transactions inner join dim_model on fact_transactions.idmodel=dim_model.idmodel
inner join dim_manufacturer on  dim_model.idmanufacturer=dim_manufacturer.idmanufacturer
where manufacturer_name in (select top 5 manufacturer_name from fact_transactions inner join dim_model on fact_transactions.idmodel=dim_model.idmodel
inner join dim_manufacturer on  dim_model.idmanufacturer=dim_manufacturer.idmanufacturer group by manufacturer_name order by sum(quantity) desc) 
group by manufacturer_name,Model_Name order by avg(totalprice) desc;


--6.	List the names of the customers and the average amount spent in 2009, where the average is higher than 500
select customer_name ,avg(totalprice) from  
dim_customer inner join fact_transactions on dim_customer.idcustomer=fact_transactions.idcustomer
inner join dim_date on dim_date.date=fact_transactions.date where year = 2009 group by customer_name having avg(totalprice)>500



--7.	List if there is any model that was in the top 5 in terms of quantity, simultaneously in 2008, 2009 and 2010
select * from 
(select top 5 model_name from fact_transactions inner join dim_model on fact_transactions.idmodel=dim_model.idmodel
inner join dim_date on fact_transactions.date=dim_date.date where year = 2008 group by model_name,year order by year,sum(quantity) desc ) dksk
intersect
select * from 
(select top 5 model_name from fact_transactions inner join dim_model on fact_transactions.idmodel=dim_model.idmodel
inner join dim_date on fact_transactions.date=dim_date.date where year = 2009 group by model_name,year order by year,sum(quantity) desc) dksk
intersect
select * from 
(select top 5 model_name from fact_transactions inner join dim_model on fact_transactions.idmodel=dim_model.idmodel
inner join dim_date on fact_transactions.date=dim_date.date where year = 2010 group by model_name,year order by year,sum(quantity) desc) dksk
...
select 2009 as year, manufacturer_name as [manufacturer with 2nd top sales],sum(totalprice) sales from FACT_TRANSACTIONS inner join DIM_Model on FACT_TRANSACTIONS.idmodel=DIM_MODEL.idmodel
inner join DIM_MANUFACTURER on DIM_MANUFACTURER.IDmanufacturer=DIM_MODEL.idmanufacturer
inner join DIM_DATE on FACT_TRANSACTIONS.date=dim_date.date where year=2009 group by  manufacturer_name having  sum(totalprice)=
(select sales from   
(select row_number() over(order by sales desc) rn,sales from
(select sum(totalprice) as sales from FACT_TRANSACTIONS inner join DIM_Model on FACT_TRANSACTIONS.idmodel=DIM_MODEL.idmodel
inner join DIM_MANUFACTURER on DIM_MANUFACTURER.IDmanufacturer=DIM_MODEL.idmanufacturer
inner join DIM_DATE on FACT_TRANSACTIONS.date=dim_date.date where year=2009 group by  manufacturer_name) as dksk) as kd where rn=2)

union

select 2010 as year, manufacturer_name as [manufacturer with 2nd top sales],SUM(totalprice) sales from FACT_TRANSACTIONS inner join DIM_Model on FACT_TRANSACTIONS.idmodel=DIM_MODEL.idmodel
inner join DIM_MANUFACTURER on DIM_MANUFACTURER.IDmanufacturer=DIM_MODEL.idmanufacturer
inner join DIM_DATE on FACT_TRANSACTIONS.date=dim_date.date where year=2010 group by  manufacturer_name having  sum(totalprice)=
(select sales from   
(select row_number() over(order by sales desc) rn,sales from
(select sum(totalprice) as sales from FACT_TRANSACTIONS inner join DIM_Model on FACT_TRANSACTIONS.idmodel=DIM_MODEL.idmodel
inner join DIM_MANUFACTURER on DIM_MANUFACTURER.IDmanufacturer=DIM_MODEL.idmanufacturer
inner join DIM_DATE on FACT_TRANSACTIONS.date=dim_date.date where year=2010 group by  manufacturer_name) as dksk) as kd where rn=2)
...
select distinct manufacturer_name from FACT_TRANSACTIONS inner join DIM_Model on FACT_TRANSACTIONS.idmodel=DIM_MODEL.idmodel
inner join DIM_MANUFACTURER on DIM_MANUFACTURER.IDmanufacturer=DIM_MODEL.idmanufacturer where YEAR(date)=2010
except
select distinct manufacturer_name from FACT_TRANSACTIONS inner join DIM_Model on FACT_TRANSACTIONS.idmodel=DIM_MODEL.idmodel
inner join DIM_MANUFACTURER on DIM_MANUFACTURER.IDmanufacturer=DIM_MODEL.idmanufacturer where YEAR(date) =2009
...
----------confusing question but i tried ------------
----------if we want to check year wise top 10 customers on basis of their average spend----------------
select rn,YEAR,customer_name,[average spend],[average quantity],case when [prev spend]=0.00 then 0 ELSE  (([average spend]-[prev spend])/([Prev Spend]))*100 end as percent_change from 
(select ROW_NUMBER() over(partition by year(date) order by avg(totalprice) desc) rn,YEAR(date) as year ,customer_name,AVG(totalprice) [average spend],AVG(quantity) as [average quantity]
,LAG(AVG(totalprice),1,0) OVER(PARTITION BY YEAR([DATE]) order by avg(totalprice) desc) [Prev Spend]from FACT_TRANSACTIONS inner join DIM_CUSTOMER on FACT_TRANSACTIONS.idcustomer=DIM_CUSTOMER.idcustomer group by YEAR(date),Customer_Name) as kasiv
where rn<11;



---------------if we want to check percentage of change in average spend for each customer in respective years---------------

 select rn,year,customer_name,[average spend],[average quantity],case when prev_spend=0.00 then 0 ELSE  (([average spend]-prev_spend)/(Prev_Spend))*100 end as percent_change from
(select row_number() over(partition by customer_name order by sum(totalprice) desc,avg(totalprice) desc) as rn ,YEAR(date) as year,customer_name ,avg(totalprice)[average spend] ,avg(quantity) as [average quantity]
,LAG(AVG(totalprice),1,0) OVER(PARTITION BY customer_name ORDER BY (YEAR([DATE]))) [Prev_Spend] from FACT_TRANSACTIONS inner join DIM_CUSTOMER on FACT_TRANSACTIONS.idcustomer=DIM_CUSTOMER.idcustomer
 group by YEAR(date) ,Customer_Name) my_derived_table where my_derived_table.rn<11 order by Customer_Name,year 
